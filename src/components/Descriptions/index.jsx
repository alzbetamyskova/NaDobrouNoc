import React from 'react';
import './style.css';

const Descriptions = () => {
  return(
    <div className='descriptions'>
      <p>Vítejte, drahé maminky, tatínkové, tetičky, starší sestry či bratrové, babičky i dědečkové, ve webové aplikaci Na dobrou noc. Máte doma, nebo na výletě děti, které nemohou večer usnout? Přečtěte jim pohádku!</p>
      <p>Zvolte pohádku náhodnou, vyberte ze seznamu nebo si najděte pohádku podle klíčového slova. Dále můžete dětem pustit audio pohádku, či pohádku z videa, namluvenou.</p>
      <p>Pojďme se do toho pustit společně, tlačítkem níže.</p>
    </div>
  );
};

export default Descriptions;